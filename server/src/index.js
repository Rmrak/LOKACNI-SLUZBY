...
const OSRM_URL = process.env.OSRM_URL || 'https://router.project-osrm.org/route/v1/foot';
...
app.get('/api/spots', (req, res) => {
  ...
  if (type === 'risk' || type === 'cool') {
    stmt = db.prepare('SELECT * FROM spots WHERE type = ? ORDER BY id DESC');
    return res.json({ spots: stmt.all(type) });
  }
  stmt = db.prepare('SELECT * FROM spots ORDER BY id DESC');
...
app.get('/api/route', async (req, res) => {
  try {
    ...
    const pr = ['foot','bike','driving'].includes(profile) ? profile : 'foot';
    const base = OSRM_URL.includes('/route/v1/') ? OSRM_URL : `https://router.project-osrm.org/route/v1/${pr}`;
